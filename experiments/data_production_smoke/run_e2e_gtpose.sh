#!/usr/bin/env bash
# End-to-end smoke test: DL3DV GT-pose mode (Stage 01→02_gtpose→05→06)
# Stage 04 (filter) is skipped: DL3DV smoke lacks UniMatch/DOVER deps.
#
# Usage: bash experiments/data_production_smoke/run_e2e_gtpose.sh [DATA_DIR]
...
OUT_DIR="/mnt/afs/davidwang/workspace/data/dl3dv_smoke_shards_gtpose"
mkdir -p "${OUT_DIR}"
...
for SCENE_DIR in "${SCENES[@]}"; do
  ...
  if [ ! -f "${SCENE_DIR}/video.mp4" ] || [ ! -f "${SCENE_DIR}/gt_poses.npy" ]; then
    echo "  [Step 0] Preparing scene (images → video.mp4 + gt poses)..."
    python "${SCRIPT_DIR}/prepare_dl3dv.py" "${SCENE_DIR}"
  else
    echo "  [Step 0] video.mp4 + gt_poses.npy already exist, skipping prepare."
  fi

  GT_POSES="${SCENE_DIR}/gt_poses.npy"
  if [ ! -f "${GT_POSES}" ]; then
    echo "ERROR: gt_poses.npy not found at ${GT_POSES}"
    exit 1
  fi
...
PYEOF
  ...
  # ── Step 2: GT-pose estimation (Pi3X structure + Umeyama-Sim3 alignment) ──
  POSE_DIR="${WORK_DIR}/pose_work"
  POSE_ARTIFACT="${WORK_DIR}/pose_artifact.npz"
  if [ ! -f "${POSE_ARTIFACT}" ]; then
    echo "  [Step 2] GT-pose estimation (Pi3X + Umeyama-Sim3 + GT trajectory)..."
    python - <<PYEOF
from sana_wm_pipeline.stage02_pose import mode_gtpose
from pathlib import Path
import numpy as np
art = mode_gtpose.run_gtpose(
    Path("${NORM_VIDEO}"),
    Path("${GT_POSES}"),
    Path("${POSE_DIR}"),
    pi3x_cmd=("python", "${PROJECT_ROOT}/scripts/pi3x_infer_cli.py"),
)
np.savez_compressed(
    "${POSE_ARTIFACT}",
