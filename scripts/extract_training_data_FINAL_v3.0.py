#!/usr/bin/env python3
"""
从 CMCC 原始 WDS 数据集中提取筛选后的训练样本（最终修正版）

v3.0 修正：
  - ✅ 修正文件扩展名（.mp4 而不是 .video.mp4）
  - ✅ 容错机制（跳过损坏的 tar）
  - ✅ 实时进度输出
  - ✅ 详细日志记录
"""
    ...
    for worker_idx, worker_dir in enumerate(worker_dirs, 1):
        ...
        for tar_idx, tar_path in enumerate(tar_files, 1):
            ...
            if all(f in members for f in required_files):
                logging.info(f"    ✅ 找到！位置: {worker_dir.name}/{tar_path.name}")
...
def extract_training_data(filtered_list, data_root, output_dir, dry_run=False):
    ...
    logging.info(f"  文件扩展名: {REQUIRED_EXTENSIONS}\n")
...
def generate_report(stats, missing_samples, output_dir):
    ...
    report = []
    report.append("=" * 70)
    report.append("CMCC 训练数据提取报告 v3.0（最终修正版）")
    report.append("=" * 70)
    report.append("")
...
def main():
    parser = argparse.ArgumentParser(
        description="从 CMCC WDS tar 分片中提取筛选后的训练样本 v3.0（最终修正版）"
    )
    ...
    logging.info("\n" + "=" * 70)
    logging.info("CMCC 训练数据提取任务 v3.0（最终修正版）")
    logging.info("修正内容：文件扩展名 .mp4（不是 .video.mp4）")
    logging.info("=" * 70 + "\n")
