#!/usr/bin/env python3
"""
从 CMCC 原始 WDS 数据集中提取筛选后的训练样本（v3.1 - caption 可选）

v3.1 修正：
  - ✅ caption.txt 设为可选文件（不影响提取成功）
  - ✅ 其他 4 个文件为必需
"""
        ...
        with tarfile.open(tar_path, 'r') as tar:
            for member_name in member_names:
                try:
                    ...
                    continue
        return len(extracted) >= len(REQUIRED_EXTENSIONS), len(extracted)
    except Exception:
        try:
            with tarfile.open(tar_path, 'r', errorlevel=0) as tar:
                for member_name in member_names:
...
def find_sample_in_tar_shards(sample_id, dataset_dir, corrupted_tars, sample_idx, total_samples):
    ...
    # 所有文件（包括可选）
    all_files = [f"{sample_id}{ext}" for ext in ALL_EXTENSIONS]
    ...
    for worker_idx, worker_dir in enumerate(worker_dirs, 1):
        ...
        # 每 10 个 worker 显示一次进度
        if worker_idx % 10 == 0:
            logging.info(f"    进度: {worker_idx}/{len(worker_dirs)} worker 已扫描")
        ...
        for tar_idx, tar_path in enumerate(tar_files, 1):
            ...
            if all(f in members for f in required_files):
                # 找到所有存在的文件（包括可选的）
                files_to_extract = [f for f in all_files if f in members]
                logging.info(f"    ✅ 找到！位置: {worker_dir.name}/{tar_path.name} ({len(files_to_extract)}/{len(all_files)} 文件)")
                return (tar_path, files_to_extract)
...
def extract_training_data(filtered_list, data_root, output_dir, dry_run=False):
    ...
    logging.info(f"  必需文件: {REQUIRED_EXTENSIONS}")
    logging.info(f"  可选文件: {OPTIONAL_EXTENSIONS}\n")
        ...
        if result:
            ...
            if not dry_run:
                ...
                if success:
                    ...
                    logging.info(f"  ✅ 提取成功 ({extracted_count} 文件)")
...
def generate_report(stats, missing_samples, output_dir):
    ...
    report = []
    report.append("=" * 70)
    report.append("CMCC 训练数据提取报告 v3.1（caption 可选版）")
    report.append("=" * 70)
    report.append("")
...
def main():
    parser = argparse.ArgumentParser(
        description="从 CMCC WDS tar 分片中提取筛选后的训练样本 v3.1（caption 可选版）"
    )
    ...
    logging.info("\n" + "=" * 70)
    logging.info("CMCC 训练数据提取任务 v3.1（caption 可选版）")
    logging.info("修正内容：caption.txt 设为可选文件")
    logging.info("=" * 70 + "\n")
