...
def _pi3():
    ...
    model = model.to(dev)  # 确保所有buffers也在同设备
...
def _moge():
    ...
    # 使用本地MoGe-2权重
    local = Path(_MOGE2_WEIGHTS)
    src = str(local / "model.pt") if (local / "model.pt").exists() else str(local)
    dev = _device()
    print(f"[_real.py] Loading MoGe-2 from {src}...", flush=True)
    model = MoGeModel.from_pretrained(src, map_location=dev).eval()
    model = model.to(dev)  # 确保所有buffers也在同设备
    print(f"[_real.py] MoGe-2 loaded successfully", flush=True)
    return model
